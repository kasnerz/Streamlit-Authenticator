...
class Authenticate:
    ...
    def _token_encode(self) -> str:
        ...
        """
        return jwt.encode(
            {"name": st.session_state["name"], "username": st.session_state["username"], "exp_date": self.exp_date},
            self.key,
            algorithm="HS256",
        )
    ...
    def login(self, form_name: str, location: str = "main") -> tuple:
        ...
        if not st.session_state["authentication_status"]:
            ...
            if not st.session_state["authentication_status"]:
                ...
                self.username = login_form.text_input("Username").lower().strip()
                        ...
                        if preauthorization:
                            if new_email in self.preauthorized["emails"]:
                                self._register_credentials(
                                    new_username, new_name, new_password, new_email, preauthorization
                                )
                                return True
                            else:
